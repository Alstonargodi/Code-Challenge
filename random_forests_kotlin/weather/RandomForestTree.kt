...
class RandomForestTree(
    ...
    // this class ).
    private fun createBootstrappedDatasets() : Array<TableFrame> {
        // ArrayList to store the bootstrapped datasets ( which are DataFrame objects )
        val dataFrames = ArrayList<TableFrame>()
        for ( i in 0 until numTrees ) {
    ...
    private fun createForest( dataFrames : Array<TableFrame> ) {
        for ( i in 0 until numTrees ) {
